...
class GameManage(object):
	...
	def __init__(self):
		...
		self.text_top_scores = None
		self.over_img =  pygame.image.load('./resource/gameover.png')
	...
	def control(self):
		...
		for event in pygame.event.get():
			...
			if self.g_hero_plane.is_over() and event.type == MOUSEBUTTONDOWN:
				#restart
				self.restart()
	...
	def display(self):
		...
		self.text_scores = self.game_font.render(str(self.g_scores),True,(0,0,0))
		self.g_window.blit(self.g_background,(0,0))
		self.g_window.blit(self.text_scores,(0,0))
		self.g_window.blit(self.pause_img,(438,0))

	def restart(self):
		#self.g_hero_plane.img_index = 0
		#self.HP = 100
		#self.g_hero_plane.hit = False
		self.g_scores = 0
		self.g_enemy_plane_list.clear()
		self.g_hero_plane.enemy_list.clear()
		self.pause_img_list.clear()
		self.initalize()

	def rw_scores(self):
		#open file and write scores
		with open('./scores.g','a') as file:
			file.write(str(self.g_scores)+'\n')
		
		#find top scores 
		max_score = 0
		with open('./scores.g','r') as file:
			for line in file.readlines():
					score = int(line)
					if max_score < score:
						max_score = score
					self.text_top_scores = self.game_font.render(str(max_score),True,(0,0,0))
...
def main():
	...
	flag = False # only one time rw_scores()
	while True:
		pygame.init()
		while not game.pause:
			game.control()
			game.display()
			if not game.g_hero_plane.is_over():
			...
			if not game.g_hero_plane.is_over():
				...
				game.g_window.blit(game.over_img,(0,0))
				#show current scores
				if not flag:
					flag = True
					game.rw_scores()
				game.g_window.blit(game.text_top_scores,(200,300))
				game.g_window.blit(game.text_scores,(200,600))
